...
pfi<- read.csv2("PFI_Metabric.csv",sep = ";", as.is = T, check.names = F)

#Merge the merged file and the dss/pfi file 
ER_DSS <- merge(merged, dss, by.x= "ID", by.y = "Patient ID")
ER_PFI <- merge(merged, pfi, by.x= "ID", by.y = "Patient ID")


# 1)Subset ER_DSS and ER_PFI based on positive ER status 
ER_DSS_pos <- subset(ER_DSS, ER_DSS$`ER Status` %in% "Positive")
ER_PFI_pos <- subset(ER_PFI, ER_PFI$`ER Status` %in% "Positive")


# 2) You can also subset the df based on HER2 negative status 
ER_DSS_her <- subset(ER_DSS_pos, ER_DSS_pos$`HER2 Status` %in% "Negative")
ER_PFI_her <- subset(ER_PFI_pos, ER_PFI_pos$`HER2 Status` %in% "Negative")
...
ER_DSS_her <- subset(ER_DSS_her, ER_DSS_her$DSS.months > 0)

ER_PFI_pos <- subset(ER_PFI_pos, ER_PFI_pos$PFI.months > 0)
ER_PFI_her <- subset(ER_PFI_her, ER_PFI_her$PFI.months > 0)


#Convert DSS/PFI.time into years 
ER_DSS_pos$years <- ER_DSS_pos$DSS.months/12
ER_DSS_her$years <- ER_DSS_her$DSS.months/12 

ER_PFI_pos$years <- ER_PFI_pos$PFI.months/12
ER_PFI_her$years <- ER_PFI_her$PFI.months/12
...
pdf("TRIM45_ER_pos_DSS_metabric.pdf", height = 8, width = 8)
print(ggsurv)
dev.off()
...
list <- list(dss_er, dss_her, pfi_er, pfi_her)
ggexport(list, filename = "TRIM45_ER_pos_HER_neg_DSS_PFI_V2.pdf", height= 15, width = 15)
