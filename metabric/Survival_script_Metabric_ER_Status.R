...
################################################################################
#Optional:check the distribution of
# GMM as cutoff: 
data_dss <- info_dss %>%
  dplyr::pull(S100A9) # gets the data we want to look at

fit_dss <- Mclust(data_dss, G = 2) # modelling of data by using two gaussian mix models --> G is number of clusters 

means <- fit_dss$parameters$mean # extracts the means of the two clusters from the fitted model 
#mean of each of the two models (in this case its two)
v <- median(data_dss)
shapiro.test(data_dss)
v_dss <- mean(means) # mean of the cluster mean
...
# GMM as cutoff: 
data_pfi <- info_pfi %>%
  dplyr::pull(STC2) # gets the data we want to look at

fit_pfi <- Mclust(data_pfi, G = 2) # modelling of data by using two gaussian mix models --> G is number of clusters 

means <- fit_pfi$parameters$mean # extracts the means of the two clusters from the fitted model 
#mean of each of the two models (in this case its two)

v_pfi <- mean(means) # mean of the cluster mean
# the value here is 4.928209 (does not directly get the minimum)
# it performs a clustering analysis and calculates the mean of the cluster means
...
ggplot(info_pfi, aes(x = STC2)) +
  ...
  geom_vline(xintercept = v_pfi, color = "#40E0D0", linetype = "dashed", size = 1.5) +  # Add vertical line for v
  labs(y = "Frequency", x = "Expression (log2+1)") +
  theme(legend.position = "none",
        plot.title = element_blank(),
        axis.text.x = element_text(size = 10),
