...
# Load Gene List #
##################
df <- read.csv2("limma_metabric_er_positive_top_bottom_t45_level_BH.csv")
...
df <- clusterProfiler::bitr(geneID = df$X,fromType="SYMBOL", toType="ENTREZID",
                            OrgDb="org.Hs.eg.db")
...
#Save the files 
pdf("GO_enrichment_metabric_for_downregulated_genes_high_vs_low_trim45_BP.pdf", width = 8, height = 10, onefile = F)
print(p)
dev.off()


png("GO_enrichment_for_downregulated_genes_high_vs_low_trim45_BP.png", res= 200, width = 1800, height = 2200)
print(p)
dev.off()
