...
# Find the position where TRIM45 goes from negative to positive
separation_position <- which(sort(test$GREB1) >= 0)[1]
number <- as.data.frame(test$GREB1 >0)

p <-ggplot(test, aes(x = reorder(id, GREB1), y = GREB1, fill = GREB1), color= NA) +
  geom_bar(stat = "identity", )+
  geom_vline(xintercept = separation_position, linetype = "dashed", color = "black", size = 0.8, alpha = 0.8) + 
...
png("edge_weight_esr1_greb1.png", width = 8, height = 6, units = "in", res = 300)
plot(p)
dev.off()

pdf("edge_weight_esr1_greb1.pdf", width = 6, height = 4)
plot(p)
dev.off()
...
png("esr1_greb1_expr_edge_weight_groups.png", width = 6, height = 6, units = "in", res = 300) 
plot(p)
dev.off()

pdf("esr1_greb1_expr_edge_weight_groups.pdf", width = 6, height = 6) 
plot(p)
dev.off()  